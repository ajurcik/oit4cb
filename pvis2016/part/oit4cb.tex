...
We call these tori as \textit{isolated}, because they do not have any neighboring triangle (see Figure~\ref{fig:isolated}).
...
In the second step, all connected components of $G$ are found and labeled using the \textcolor{red}{BFS} algorithm.
...
An \textit{isolated} torus must be handled in two ways.
...
Second, each isolated torus should clip the overlaid fragments of its spherical patch (see \textcolor{red}{Figure~\ref{fig:isolated}}).

\begin{figure}[htb]
  \centering
  \includegraphics[width=2in]{image/isolated-cutaway.png}
  \caption{Isolated torus between two spheres.
	Isolated tori do not have neighboring triangles, therefore the surface component to which they belong can not be determined directly from the \textit{surface graph}.
	An isolated torus cuts a circular hole into its neighboring spherical patches. \textcolor{red}{TODO image}
	}
	\label{fig:isolated}
\end{figure}

While preparing toroidal patches for ray-casting, an \textit{isolated} torus can be encountered.
We add this torus into the buffer for ray-casting and we mark it for later processing.
...
\begin{enumerate}
  \item One of the spheres forms only one patch. Then, the torus lies in the same surface component as this patch.
	\item Both spheres form two or more patches. We find neighboring patch of the torus by employing the point in spherical patch test (see Section~\ref{sec:spherical-patches}). Then, the torus lies in the same surface component as its neighboring patch.
\end{enumerate}
