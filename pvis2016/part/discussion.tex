...
The main limitations of our method are its applicability to large data and, the precision of the surface area calculation.
When applied to the molecules consisting of more than 10000 of atoms, the performance of the method limits the interactivity.
We anticipate that the performance can be improved by applying the transparent SES only to the interesting parts of the molecule, while using another visualization method for the rest of it.
We expect the proper area calculation, e.g., by applying the method presented by Connolly~\cite{connolly1983analytical}, to be done as a future work.
The single-pixel artifacts (Sec.~\ref{sec:spherical-patches}) in the final image also limit the method in terms of creating images for publications.
